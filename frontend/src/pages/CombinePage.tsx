...
const tools = [
  ...
  },
  {
    ...
    description: "Compress or expand images to specific dimensions and file size",
    category: "Optimize",
    icon: Maximize2,
    iconBg: "bg-amber-50 dark:bg-amber-900/30",
    iconColor: "text-amber-600",
    accept: "image/*",
    multiple: false,
    endpoint: "/combine/image-compress/",
    historyType: "image_compress",
  },
];
...
export default function CombinePage() {
  ...
  const [compressOpen, setCompressOpen] = useState(true);
  ...
  const [toolOptions, setToolOptions] = useState<Record<string, any>>({
    ...
    image_compress_mode: "compress",
    image_target_width: "",
    image_target_height: "",
    image_size_mode: "exact",
    image_target_size_min: "",
    image_target_size_max: "",
    image_size_unit: "KB",
  });
  ...
  const isImageCompressor = activeTool?.name === "Image Compressor";

  const isMergeTool = activeTool?.category === "Merge";
  const canReorderFiles = Boolean(activeTool?.multiple && isMergeTool);
  ...
  const validation = useMemo(() => {
    ...
    if (isImageCompressor) {
      if (selectedFiles.length === 0) return { valid: false, message: "Please upload an image." };
      if (toolOptions.image_size_mode === "range") {
        const min = parseFloat(toolOptions.image_target_size_min);
        const max = parseFloat(toolOptions.image_target_size_max);
        if (!isNaN(min) && !isNaN(max) && min >= max) {
          return { valid: false, message: "Min size must be less than max size." };
        }
      }
      return { valid: true, message: `✓ Image ready to process` };
    }
  ...
  }, [activeTool, selectedFiles, pageOrder, isOrganize, isWatermark, isSplit, isImageCompressor, toolOptions.watermark_pages, toolOptions.watermark_custom_pages, toolOptions.split_parity, toolOptions.split_pages, toolOptions.image_size_mode, toolOptions.image_target_size_min, toolOptions.image_target_size_max]);
  ...
  const handleFiles = async (files: FileList | File[] | null) => {
    ...
    setSelectedFiles((previous) => {
      if (activeTool && !activeTool.multiple) return [array[array.length - 1]];
      return [...previous, ...array].slice(0, MAX_MERGE_FILES);
    });
    setDraggedFileIndex(null);
    setDragOverFileIndex(null);
    ...
    if (activeTool?.name === "Image Compressor") setCompressOpen(true);
  };
  ...
  MERGE FILE DRAG REORDER
  ========================================================= */
  const handleFileDragStart = (event: React.DragEvent<HTMLDivElement>, index: number) => {
  ...
  const handleFileDragOver = (event: React.DragEvent<HTMLDivElement>, index: number) => {
    if (!canReorderFiles || draggedFileIndex === null) return;
    event.preventDefault();
    event.dataTransfer.dropEffect = "move";
    if (draggedFileIndex !== index) setDragOverFileIndex(index);
  };

  const handleFileDrop = (event: React.DragEvent<HTMLDivElement>, targetIndex: number) => {
    if (!canReorderFiles) return;
    event.preventDefault();
    const rawIndex = event.dataTransfer.getData("text/plain");
    let sourceIndex = Number(rawIndex);
    if (!Number.isInteger(sourceIndex)) sourceIndex = draggedFileIndex ?? -1;
    
    if (sourceIndex < 0 || sourceIndex >= selectedFiles.length || targetIndex < 0 || targetIndex >= selectedFiles.length) {
      setDraggedFileIndex(null); setDragOverFileIndex(null); return;
    }
    if (sourceIndex !== targetIndex) {
      ...
      showToast("success", `File moved to position #${targetIndex + 1}`);
    }
    setDraggedFileIndex(null);
    setDragOverFileIndex(null);
  };

  const handleFileDragEnd = () => { setDraggedFileIndex(null); setDragOverFileIndex(null); };

  const moveFileUp = (index: number) => {
    if (!canReorderFiles || index <= 0) return;
    setSelectedFiles((previous) => {
      const next = [...previous];
      [next[index - 1], next[index]] = [next[index], next[index - 1]];
      return next;
    });
    showToast("success", `Moved file to position #${index}`);
  };

  const moveFileDown = (index: number) => {
    if (!canReorderFiles || index >= selectedFiles.length - 1) return;
    setSelectedFiles((previous) => {
      const next = [...previous];
      [next[index], next[index + 1]] = [next[index + 1], next[index]];
      return next;
    });
    showToast("success", `Moved file to position #${index + 2}`);
  };
  ...
  const handleBack = () => {
    setActiveTool(null); setSelectedFiles([]); setProcessedBlob(null); setProcessedFilename(""); setStatusMsg(null); setProcessStep("");
    setDraggedFileIndex(null); setDragOverFileIndex(null); clearOrganizeState();
    setEnhanceOpen(true); setWatermarkOpen(true); setSplitOpen(true); setNupOpen(true); setOrganizeOpen(true); setCompressOpen(true);
  };

  const handleProcessAnother = () => {
    setSelectedFiles([]); setProcessedBlob(null); setProcessedFilename(""); setStatusMsg(null); setProcessStep("");
    setDraggedFileIndex(null); setDragOverFileIndex(null); clearOrganizeState();
    setEnhanceOpen(true); setWatermarkOpen(true); setSplitOpen(true); setNupOpen(true); setOrganizeOpen(true); setCompressOpen(true);
  };
  ...
  const downloadFromHistory = async (item: HistoryItem) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}${cleanUrl}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleProcess = async () => {
    ...
    // ✅ NEW: Image Compressor Logic
    if (activeTool.name === "Image Compressor") {
      formData.append("compress_mode", toolOptions.image_compress_mode);
      if (toolOptions.image_target_width) formData.append("target_width", toolOptions.image_target_width);
      if (toolOptions.image_target_height) formData.append("target_height", toolOptions.image_target_height);
      formData.append("size_mode", toolOptions.image_size_mode);
      formData.append("size_unit", toolOptions.image_size_unit);
      
      if (toolOptions.image_size_mode === "exact" && toolOptions.image_target_size_min) {
        formData.append("target_size", toolOptions.image_target_size_min);
      } else if (toolOptions.image_size_mode === "range") {
        if (toolOptions.image_target_size_min) formData.append("target_size_min", toolOptions.image_target_size_min);
        if (toolOptions.image_target_size_max) formData.append("target_size_max", toolOptions.image_target_size_max);
      }
    }
    ...
    try {
      ...
      if (cleanDownloadUrl.startsWith('/api/')) cleanDownloadUrl = cleanDownloadUrl.substring(4);
      ...
      setSelectedFiles([]); setDraggedFileIndex(null); setDragOverFileIndex(null); setProcessStep("");
    ...
    <div className="min-h-screen bg-[#f8fafc] text-[#101828] dark:bg-[#0f172a] dark:text-slate-100">
      ...
      {isImageCompressor && selectedFiles.length > 0 && !processedBlob && (
        <>
          ...
          <aside className={`fixed right-4 top-1/2 z-[350] w-[380px] max-w-[calc(100vw-32px)] -translate-y-1/2 rounded-2xl bg-white dark:bg-slate-800 shadow-2xl transition-all duration-500 ${compressOpen ? "translate-x-0 opacity-100" : "translate-x-[calc(100%+32px)] opacity-0 pointer-events-none"}`}>
            <button onClick={() => setCompressOpen(false)} className="absolute -left-3 top-6 flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-md">
              <ChevronLeft className="h-4 w-4" />
            </button>
            <div className="p-5 max-h-[calc(100vh-120px)] overflow-y-auto custom-scrollbar">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <h4 className="text-base font-bold">Image Compressor</h4>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Resize and control file size</p>
                </div>
              </div>
              ...
              <div className="space-y-5">
                {/* Mode Toggle */}
                <div>
                  <label className="mb-2 flex items-center gap-2 text-sm font-bold"><Layers className="h-4 w-4 text-amber-500" />MODE</label>
                  <div className="relative grid grid-cols-2 rounded-xl bg-slate-100 dark:bg-slate-700 p-1">
                    <div className={`absolute inset-y-1 w-[calc(50%-4px)] rounded-lg bg-white dark:bg-slate-800 shadow transition-transform duration-300 ${toolOptions.image_compress_mode === "expand" ? "translate-x-[calc(100%+4px)]" : "translate-x-0"}`} />
                    <button type="button" onClick={() => setToolOptions({ ...toolOptions, image_compress_mode: "compress" })} className={`relative z-10 flex items-center justify-center gap-2 rounded-lg py-2.5 text-xs font-bold ${toolOptions.image_compress_mode === "compress" ? "text-amber-600" : "text-slate-500"}`}>Compress</button>
                    <button type="button" onClick={() => setToolOptions({ ...toolOptions, image_compress_mode: "expand" })} className={`relative z-10 flex items-center justify-center gap-2 rounded-lg py-2.5 text-xs font-bold ${toolOptions.image_compress_mode === "expand" ? "text-amber-600" : "text-slate-500"}`}>Expand</button>
                  </div>
                </div>

                {/* Dimensions */}
                <div>
                  <label className="mb-2 flex items-center gap-2 text-sm font-bold"><Maximize2 className="h-4 w-4 text-amber-500" />DIMENSIONS (px)</label>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <span className="text-[10px] text-slate-500 mb-1 block">Width</span>
                      <input type="number" value={toolOptions.image_target_width} onChange={(e) => setToolOptions({ ...toolOptions, image_target_width: e.target.value })} placeholder="Auto" className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700 px-3 py-2.5 text-sm" />
                    </div>
                    <div>
                      <span className="text-[10px] text-slate-500 mb-1 block">Height</span>
                      <input type="number" value={toolOptions.image_target_height} onChange={(e) => setToolOptions({ ...toolOptions, image_target_height: e.target.value })} placeholder="Auto" className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700 px-3 py-2.5 text-sm" />
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative grid grid-cols-2 rounded-xl bg-slate-100 dark:bg-slate-700 p-1 mb-3">
                    <div className={`absolute inset-y-1 w-[calc(50%-4px)] rounded-lg bg-white dark:bg-slate-800 shadow transition-transform duration-300 ${toolOptions.image_size_mode === "range" ? "translate-x-[calc(100%+4px)]" : "translate-x-0"}`} />
                    <button type="button" onClick={() => setToolOptions({ ...toolOptions, image_size_mode: "exact" })} className={`relative z-10 py-2.5 text-xs font-bold ${toolOptions.image_size_mode === "exact" ? "text-amber-600" : "text-slate-500"}`}>Exact Size</button>
        ...
        <>
          ...
          <aside className={`fixed right-4 top-1/2 z-[350] w-[400px] max-w-[calc(100vw-32px)] -translate-y-1/2 rounded-2xl border border-indigo-100 dark:border-indigo-900/50 bg-white dark:bg-slate-800 shadow-2xl shadow-slate-900/15 transition-[transform,opacity] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${nupOpen ? "translate-x-0 opacity-100" : "translate-x-[calc(100%+32px)] opacity-0 pointer-events-none"}`}>
            ...
            <div className="p-5 max-h-[calc(100vh-120px)] overflow-y-auto custom-scrollbar">
              ...
              <div className="mb-5">
                <label className="mb-2 flex items-center gap-2 text-sm font-bold text-slate-700 dark:text-slate-300"><Files className="h-4 w-4 text-indigo-500" />SOURCE TYPE</label>
                <div className="relative grid grid-cols-2 rounded-xl bg-slate-100 dark:bg-slate-700 p-1">
                  ...
                  <button type="button" onClick={() => changeNupMode("pdf")} className={`relative z-10 flex items-center justify-center gap-2 rounded-lg py-2.5 text-xs font-bold ${nupMode === "pdf" ? "text-indigo-600 dark:text-indigo-400" : "text-slate-500"}`}><FileText className="h-4 w-4" />PDF Pages</button>
                  <button type="button" onClick={() => changeNupMode("images")} className={`relative z-10 flex items-center justify-center gap-2 rounded-lg py-2.5 text-xs font-bold ${nupMode === "images" ? "text-indigo-600 dark:text-indigo-400" : "text-slate-500"}`}><ImagePlus className="h-4 w-4" />Images</button>
                </div>
              </div>
              <div className="mb-5">
                    ...
                    return (
                      <button key={n} type="button" onClick={() => setToolOptions({ ...toolOptions, nup_pages: n })} className={`relative flex flex-col items-center justify-center rounded-xl border-2 p-2.5 transition-all ${active ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30 shadow-sm" : "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:border-indigo-300 dark:hover:border-indigo-600"}`}>
                        <div className="grid gap-[1.5px] w-8 h-6 mb-1" style={{ gridTemplateColumns: `repeat(${cols}, 1fr)`, gridTemplateRows: `repeat(${rows}, 1fr)` }}>
                          {Array.from({ length: n }).map((_, i) => (<div key={i} className={`rounded-[1px] ${active ? "bg-indigo-500" : "bg-slate-300 dark:bg-slate-600"}`} />))}
                        </div>
              ...
              <div className="mb-5">
                <label className="mb-2 flex items-center gap-2 text-sm font-bold text-slate-700 dark:text-slate-300"><Layout className="h-4 w-4 text-indigo-500" />LAYOUT</label>
                <div className="grid grid-cols-3 gap-2">
                  {[{ value: "grid", label: "Grid", icon: Grid3x3 }, { value: "horizontal", label: "Horizontal", icon: RectangleHorizontal }, { value: "vertical", label: "Vertical", icon: RectangleVertical }].map((option) => {
              ...
              <div className="mb-5">
                <label className="mb-2 flex items-center gap-2 text-sm font-bold text-slate-700 dark:text-slate-300"><Square className="h-4 w-4 text-indigo-500" />PAGE SIZE</label>
                <div className="grid grid-cols-3 gap-2">
                  {["a4", "letter", "a3"].map((size) => {
                    const active = toolOptions.nup_page_size === size;
                    return (
                      <button key={size} type="button" onClick={() => setToolOptions({ ...toolOptions, nup_page_size: size })} className={`rounded-xl border-2 py-2.5 text-xs font-bold uppercase transition-all ${active ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400" : "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400"}`}>{size}</button>
                    );
                  })}
                </div>
              </div>
              <div className="mb-5">
                <label className="mb-2 flex items-center gap-2 text-sm font-bold text-slate-700 dark:text-slate-300"><Maximize2 className="h-4 w-4 text-indigo-500" />ORIENTATION</label>
                <div className="grid grid-cols-2 gap-2">
                  {[{ value: "portrait", label: "Portrait", icon: RectangleVertical }, { value: "landscape", label: "Landscape", icon: RectangleHorizontal }].map((option) => {
              ...
              <div className="mb-5">
                <label className="mb-2 flex items-center gap-2 text-sm font-bold text-slate-700 dark:text-slate-300"><Layers className="h-4 w-4 text-indigo-500" />SPACING</label>
                <div className="grid grid-cols-4 gap-1.5 rounded-xl bg-slate-100 dark:bg-slate-700 p-1">
              ...
              <div className="mb-5">
                <label className="mb-2 flex items-center gap-2 text-sm font-bold text-slate-700 dark:text-slate-300"><Square className="h-4 w-4 text-indigo-500" />BORDER</label>
                <div className="relative grid grid-cols-2 rounded-xl bg-slate-100 dark:bg-slate-700 p-1">
              ...
              <div className="mb-5 rounded-xl border border-indigo-100 dark:border-indigo-900/50 bg-indigo-50/30 dark:bg-indigo-900/10 p-4">
                <div className="mb-3 flex items-center gap-2"><Eye className="h-4 w-4 text-indigo-500" /><span className="text-sm font-bold text-slate-900 dark:text-slate-100">LIVE PREVIEW</span></div>
                <div className="flex items-center justify-center rounded-lg border-2 border-dashed border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
                  <div className="grid gap-1 bg-slate-100 dark:bg-slate-700 p-2 rounded-lg" style={{ gridTemplateColumns: `repeat(${nupColsRows[0]}, 1fr)`, gridTemplateRows: `repeat(${nupColsRows[1]}, 1fr)`, width: toolOptions.nup_layout === "vertical" ? "60px" : "140px", height: toolOptions.nup_layout === "horizontal" ? "60px" : "100px" }}>
                    {Array.from({ length: toolOptions.nup_pages }).map((_, i) => (<div key={i} className="rounded-sm bg-indigo-400 dark:bg-indigo-500" />))}
                  </div>
                </div>
                <p className="mt-2 text-center text-[10px] text-slate-500 dark:text-slate-400">{toolOptions.nup_pages} pages per sheet • {toolOptions.nup_layout} • {toolOptions.nup_page_size.toUpperCase()}</p>
              </div>
              <div className="rounded-xl border border-indigo-100 dark:border-indigo-900/50 bg-indigo-50/70 dark:bg-indigo-900/20 p-4">
                <div className="flex items-start gap-3">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400"><Grid3x3 className="h-5 w-5" /></div>
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-indigo-700 dark:text-indigo-400">N-up Configuration</p>
                    <p className="mt-1 text-[10px] leading-4 text-indigo-600 dark:text-indigo-500">{selectedFiles.length} {nupMode === "pdf" ? "PDF pages" : "images"} → {toolOptions.nup_pages} per sheet</p>
                    <p className="mt-0.5 text-[10px] text-indigo-500 dark:text-indigo-500/70">{Math.ceil(selectedFiles.length / toolOptions.nup_pages)} output sheet{Math.ceil(selectedFiles.length / toolOptions.nup_pages) > 1 ? "s" : ""}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        {!activeTool ? (
            ...
            <div className="mt-6 flex justify-center">
              <div className="flex gap-1 overflow-x-auto rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-1.5 shadow-sm">
                {categories.map((category) => (
                  <button key={category} type="button" onClick={() => setActiveCategory(category)} className={`shrink-0 rounded-xl px-4 py-2 text-xs font-medium ${activeCategory === category ? "bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400" : "text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700"}`}>{category}</button>
                ))}
              </div>
            </div>
          ...
          <section className="mx-auto max-w-3xl">
            <div className="mb-6 flex items-center justify-between">
              <button type="button" onClick={handleBack} className="flex items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-2 text-sm font-semibold shadow-sm"><ChevronLeft className="h-4 w-4" />Back</button>
            ...
            {isOrganize && selectedFiles.length > 0 && !processedBlob && (
              <div className="mt-5 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-sm">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => removeFile(0)} className="text-xs font-semibold text-red-500">Change File</button>
                </div>
                {isLoadingPages ? (
              ...
              <div className="mt-5 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    {canReorderFiles && <p className="mt-1 flex items-center gap-1 text-[10px] font-medium text-blue-500"><GripVertical className="h-3 w-3" />Drag files to change merge order</p>}
                  </div>
                  <button type="button" onClick={clearSelectedFiles} className="text-xs font-semibold text-red-500">Clear All</button>
                </div>

                {canReorderFiles && (
                  <div className="mt-4 flex items-center justify-between rounded-xl border border-blue-100 dark:border-blue-900/40 bg-blue-50 dark:bg-blue-900/20 px-3 py-2.5">
                    <div className="flex items-center gap-2"><Layers className="h-4 w-4 text-blue-600 dark:text-blue-400" /><span className="text-[10px] font-bold text-blue-700 dark:text-blue-400">Merge order</span></div>
                    <span className="text-[10px] font-medium text-blue-500 dark:text-blue-400">Top → Bottom</span>
                  </div>
                )}

                <div className="mt-4 space-y-2">
                  {selectedFiles.map((file, index) => {
                    const isDraggingFile = draggedFileIndex === index;
                    const isDropTarget = dragOverFileIndex === index && draggedFileIndex !== index;
                    const isExcel = file.name.toLowerCase().endsWith(".xlsx") || file.name.toLowerCase().endsWith(".xls");
                    return (
                      <React.Fragment key={`${file.name}-${file.size}-${index}`}>
                        {canReorderFiles && isDropTarget && <div className="h-1.5 rounded-full bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.35)]" />}
                        <div draggable={canReorderFiles} onDragStart={canReorderFiles ? (e) => handleFileDragStart(e, index) : undefined} onDragOver={canReorderFiles ? (e) => handleFileDragOver(e, index) : undefined} onDrop={canReorderFiles ? (e) => handleFileDrop(e, index) : undefined} onDragEnd={canReorderFiles ? handleFileDragEnd : undefined} className={`group flex items-center gap-3 rounded-xl border-2 p-3 transition-all ${isDraggingFile ? "border-blue-400 bg-blue-50 dark:bg-blue-900/30 scale-[0.985] opacity-50 shadow-lg" : isDropTarget ? "border-blue-300 bg-blue-50/40 dark:bg-blue-900/20" : "border-transparent bg-slate-50 dark:bg-slate-700/50 hover:border-slate-200 dark:hover:border-slate-600"} ${canReorderFiles ? "cursor-grab active:cursor-grabbing" : ""}`}>
                          {canReorderFiles && <div className="flex h-10 w-8 shrink-0 items-center justify-center rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 cursor-grab active:cursor-grabbing" title="Drag to reorder"><GripVertical className="h-5 w-5 text-slate-400 group-hover:text-blue-500" /></div>}
                          {canReorderFiles && <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-blue-50 dark:bg-blue-900/30 text-[11px] font-bold text-blue-600 dark:text-blue-400">{index + 1}</div>}
                          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                            {isExcel ? <FileSpreadsheet className="h-4 w-4 text-green-600" /> : file.type.startsWith("image/") ? <FileImage className="h-4 w-4 text-purple-500" /> : <FileText className="h-4 w-4 text-blue-500" />}
                          </div>
                          <div className="min-w-0 flex-1">
                            <p className="truncate text-xs font-semibold">{file.name}</p>
                            <p className="text-[10px] text-slate-400">{formatBytes(file.size)}</p>
                            {canReorderFiles && <p className="mt-0.5 text-[9px] font-semibold text-blue-500 dark:text-blue-400">Merge Position #{index + 1}</p>}
                          </div>
                          {canReorderFiles && (
                            <div className="flex flex-col gap-1">
                              <button type="button" onClick={() => moveFileUp(index)} disabled={index === 0} title="Move up" className="flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-500 transition hover:border-blue-300 hover:text-blue-500 disabled:cursor-not-allowed disabled:opacity-30"><ChevronUp className="h-3.5 w-3.5" /></button>
                              <button type="button" onClick={() => moveFileDown(index)} disabled={index === selectedFiles.length - 1} title="Move down" className="flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-500 transition hover:border-blue-300 hover:text-blue-500 disabled:cursor-not-allowed disabled:opacity-30"><ChevronDown className="h-3.5 w-3.5" /></button>
                            </div>
                          )}
                          <button type="button" onClick={() => removeFile(index)} title="Remove file" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-slate-400 transition hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/20"><X className="h-4 w-4" /></button>
                        </div>
                      </React.Fragment>
                    );
                  })}
                ...
                {canReorderFiles && selectedFiles.length >= 2 && (
                  <div className="mt-4 rounded-xl border border-blue-100 dark:border-blue-900/40 bg-blue-50/60 dark:bg-blue-900/10 p-3">
                    <div className="flex items-start gap-2">
                      <Info className="mt-0.5 h-4 w-4 shrink-0 text-blue-500" />
                      <div>
                        <p className="text-[10px] font-bold text-blue-700 dark:text-blue-400">Final merge sequence</p>
                        <p className="mt-1 text-[9px] leading-4 text-blue-600 dark:text-blue-400">Files will be merged from <span className="font-bold">Position #1</span> to <span className="font-bold">Position #{selectedFiles.length}</span>.</p>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="mt-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {[{ value: "low", label: "High Compression" }, { value: "medium", label: "Balanced" }, { value: "high", label: "High Quality" }].map((option) => (
                        <button key={option.value} type="button" onClick={() => setToolOptions({ ...toolOptions, quality: option.value })} className={`rounded-lg border-2 p-3 text-left ${toolOptions.quality === option.value ? "border-blue-500 bg-blue-50 dark:bg-blue-900/30" : "border-slate-200 dark:border-slate-700"}`}><p className="text-[10px] font-bold">{option.label}</p></button>
                      ))}
                    </div>
                  </div>
                )}
                  ...
                  <div className="mt-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {[90, 180, 270, 360].map((angle) => (
                        <button key={angle} type="button" onClick={() => setToolOptions({ ...toolOptions, rotation: angle })} className={`rounded-lg border-2 p-3 ${toolOptions.rotation === angle ? "border-cyan-500 bg-cyan-50 dark:bg-cyan-900/30" : "border-slate-200 dark:border-slate-700"}`}><RotateCw className="mx-auto mb-1 h-5 w-5" /><p className="text-xs font-bold">{angle}°</p></button>
                      ))}
                    </div>
                  </div>
                )}
                ...
                <button type="button" onClick={handleProcess} disabled={isProcessing || !validation.valid || (isEnhance && activeEffects.length === 0)} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 py-3 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50">
              ...
              <div className="mt-6 rounded-2xl border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 p-6 text-center">
                ...
                <div className="mt-5 flex flex-wrap justify-center gap-3">
                  {downloadFormats.map((format) => {
                    ...
                    return (
                      <button key={format} type="button" onClick={() => downloadAs(format)} disabled={isExtracting} className="flex items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-5 py-2.5 text-sm font-semibold disabled:opacity-50">
                        {isExtracting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Icon className="h-4 w-4" />}{Meta.label}
                      </button>
                    );
                  })}
                </div>
                <button type="button" onClick={handleProcessAnother} className="mt-6 inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-xs font-semibold text-white"><RefreshCw className="h-3.5 w-3.5" />Process another file</button>
              </div>
            )}
            ...
            <div className="mt-6 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={refreshHistory} className="flex items-center gap-1 rounded-lg px-2 py-1 text-[10px] font-semibold text-slate-500"><RefreshCw className="h-3 w-3" />Refresh</button>
              </div>
              <div className="mt-4 space-y-2">
                {history.length === 0 ? (
                  ...
                  sortedHistory.map((item) => (
                    <div key={item.id} className="flex items-center gap-3 rounded-lg border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/50 p-3">
                      ...
                      <button type="button" onClick={() => downloadFromHistory(item)} className="rounded-lg bg-blue-600 px-3 py-1.5 text-[10px] font-semibold text-white"><Download className="inline h-3 w-3" /> Download</button>
                      <button type="button" onClick={(e) => openMenu(e, item)} className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400"><MoreVertical className="h-4 w-4" /></button>
                    </div>
                  ))
                )}
              </div>
      ...
      {activeMenuId && menuPosition && currentMenuItem && (
        <div ref={menuRef} style={{ position: "fixed", top: menuPosition.top, right: menuPosition.right }} className="z-[200] w-48 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 py-1 shadow-2xl">
          <button type="button" onClick={() => openShareModal(currentMenuItem)} className="flex w-full items-center gap-2 px-3 py-2 text-xs hover:bg-blue-50 dark:hover:bg-blue-900/30"><Share2 className="h-3.5 w-3.5" />Share</button>
          <button type="button" onClick={() => togglePin(currentMenuItem.id)} className="flex w-full items-center gap-2 px-3 py-2 text-xs hover:bg-amber-50 dark:hover:bg-amber-900/30"><Pin className="h-3.5 w-3.5" />Pin to Top</button>
          <div className="my-1 border-t border-slate-100 dark:border-slate-700" />
          <button type="button" onClick={() => deleteHistoryItem(currentMenuItem)} className="flex w-full items-center gap-2 px-3 py-2 text-xs text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30"><Trash2 className="h-3.5 w-3.5" />Delete</button>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4" onClick={() => setShowShareModal(false)}>
          <div className="w-full max-w-md rounded-2xl bg-white dark:bg-slate-800 p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => setShowShareModal(false)}><X className="h-5 w-5 text-slate-400" /></button>
            </div>
            ...
            <div className="mt-4 flex gap-2">
              ...
              <button type="button" onClick={() => copyShareLink(shareItem)} className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white">{copiedLink ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}</button>
            </div>
            <div className="mt-5 grid grid-cols-4 gap-2">
              <button type="button" onClick={() => shareToWhatsApp(shareItem)} className="rounded-xl border border-slate-200 dark:border-slate-700 p-3"><WhatsAppIcon className="mx-auto h-5 w-5 text-green-500" /><span className="mt-1 block text-[9px]">WhatsApp</span></button>
              <button type="button" onClick={() => shareToTwitter(shareItem)} className="rounded-xl border border-slate-200 dark:border-slate-700 p-3"><TwitterIcon className="mx-auto h-5 w-5" /><span className="mt-1 block text-[9px]">Twitter</span></button>
              <button type="button" onClick={() => copyShareLink(shareItem)} className="rounded-xl border border-slate-200 dark:border-slate-700 p-3"><Link2 className="mx-auto h-5 w-5 text-blue-500" /><span className="mt-1 block text-[9px]">Copy</span></button>
              <button type="button" onClick={() => downloadFromHistory(shareItem)} className="rounded-xl border border-slate-200 dark:border-slate-700 p-3"><Download className="mx-auto h-5 w-5 text-purple-500" /><span className="mt-1 block text-[9px]">Download</span></button>
            </div>
          </div>
        </div>
      )}
